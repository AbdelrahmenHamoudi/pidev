...
public class PromotionService {
     ...
     */
    public Promotion createFromAiSuggestion(PackSuggestionDTO dto) {
        java.time.LocalDate today = java.time.LocalDate.now();

        // ✅ Créer la promotion avec le constructeur approprié
        Promotion p = new Promotion();
        p.setName(dto.getSuggestedName());
        p.setDescription(dto.getSuggestedDescription());
        p.setDiscountPercentage(dto.getSuggestedDiscount());
        p.setDiscountFixed(null);
        p.setStartDate(java.sql.Date.valueOf(today));
        p.setEndDate(java.sql.Date.valueOf(today.plusMonths(3)));
        p.setPack(true);
        p.setLocked(false);
        p.setNbVues(0);
        p.setNbReservations(0);
