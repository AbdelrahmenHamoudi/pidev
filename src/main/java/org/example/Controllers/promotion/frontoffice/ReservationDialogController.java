...
import org.example.Services.promotion.*;                      // ✅ (majuscule S)
 ...
 *
 * Session user is read from UserSession (dynamic, set by User module on login).
 */
public class ReservationDialogController {
    ...
    /**
     * ✅ Méthode pour définir la promotion
     */
    ...
    private void handleConfirmer() {
        if (currentUser == null) {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Vous devez être connecté pour réserver.");
            return;
        }
